    ...
    try {
      ...
      console.log("active -> " + response.data.active);
        if (response.data.active === true) {
          setActive(false);
          if (response.data.token != null && response.data.token != "")  localStorage.setItem('jwt-token', response.data.token);
          ...
          <form className="modal__form">
            <div className="modal__form-block">
              <div className="modal__form-input">
                <input className="modal__input" type="text" maxLength="1" placeholder="A" onInput={event => {
                    handleChange(0, event.target.value);
                    jumpInput(event.target)}
                }/>
                <input className="modal__input" type="text" maxLength="1" placeholder="3" onInput={event => {
                    handleChange(1, event.target.value);
                    jumpInput(event.target)}
                }/>
                <input className="modal__input" type="text" maxLength="1" placeholder="5" onInput={event => {
                    handleChange(2, event.target.value);
                    jumpInput(event.target)}
                }/>
                <input className="modal__input" type="text" maxLength="1" placeholder="2" onInput={event => {
                    handleChange(3, event.target.value);
                    jumpInput(event.target)}
                }/>
                <input className="modal__input" type="text" maxLength="1" placeholder="1" onInput={event => {
